...
object AdventureCollection {
    ...
    fun mordProgress(entries: List<SessionHistoryEntry>) = LocalBossProgress(
        entries.distinctBy { it.sessionId }.filter { it.expedition == Expedition.ABYSS }.sumOf { it.creditedMinutes.coerceAtLeast(0) },
    )
}
